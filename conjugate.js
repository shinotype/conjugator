...
$(document).ready(function () {
  ...
  function debugMod(term, mod, w, premods) {
    ...
    if (desc.length) {
      w.append(q.word + ' - ' + desc.join(', ')).append('<br />');
    }
  }
  ...
  genOpts('adjective-options', [['い adjectives', 'iadj']]);
...
function Question(term) {
  if (!(this instanceof Question)) return new Question(term);

  this.word = term.word;
  this.kanji = '';
  if ($('#opt-kanji:checked').length == 1) this.kanji = term.kanji;
...
Question.prototype.modify = function (modSet, skipNext) {
  ...
  if (this.kanji) this.kanji = modifier.modFunc(this.kanji);
...
function pickType() {
  ...
  if (sets == null) {
    ...
    if ($('#opt-godan:checked').length) sets.push([GODAN, godan, '[godan] v.']);
  ...
  var i = 0;
  do {
    if (rando < sets[i][1].length) return sets[i];
...
function addWell(actual, expected, rootword, isCorrect) {
  var mods = $('#well').data('mods').filter(filterFalse).join(', ');

  var def = $('#meaning').text();
  if (!def) return;

  var w = $('<div/>').addClass('wellitem').data({
    type: termType,
    term: rootword,
  });
  ...
  var expected_link = $('<a/>')
    ...
    .attr({
      ...
      title:
        'Jisho - ' +
        rootword +
        ' - click Show Inflections to review conjugations.',
    });

  var wellLeft = $('<div />').addClass('well-left').append(expected_link);
  ...
  if (isCorrect) {
    ...
    w.addClass('skipped').append(wellLeft);
  }

  w.append($('<div/>').addClass('clear'));
...
function setConfig(str) {
  ...
  $('#option-menu input:checkbox').each(function () {
...
function filterMod(mod) {
  var i,
    flags = mod.flag;
  if (!checkConfig(flags)) return false;
